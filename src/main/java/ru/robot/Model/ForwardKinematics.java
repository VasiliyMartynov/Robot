...
public class ForwardKinematics {
    ...
    public static RMatrix FKinBody(RMatrix M, List<Vector6> Blist, List<BigDecimal> thetalist){
        ...
        LOGGER.debug("FKinBody input M'{}", M.getData());
        LOGGER.debug("FKinBody input Blist'{}", Blist);
        LOGGER.debug("FKinBody input thetalist'{}", thetalist);
        ...
        for(int i = 0; i <= thetalist.size() - 1; i++){
            T = RMatrix.mult(
                    T, MatrixExp6(
                            VecTose3(
                                    Vector6.mult(Blist.get(i), thetalist.get(i)
                                    )
                            )
                    )
            );
